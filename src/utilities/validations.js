...
const generateInvalidDataTypeMessage = (key, dataType, fieldText) => {
  return { [key]: `${fieldText} should be of type ${dataType}` };
};
...
const generateInvalidEnumMessage = (field, enums) => {
  return { "any.only": `${field} should be one of ${enums}` };
};
/**
 * 
...
const generateMandatoryFieldMessage = (message) => {
   return `${message} is a mandatory field`;
 }
